...
def potential_pitfalls():
    ...
    {{
        "触发条件": "当用户提问中涉及到香油等经过加工的植物油是否适用农产品抵扣政策时", 
        "操作指引": "香油属于“食用植物油”，不属于《农业产品征税范围注释》中的“初级农产品”。虽然两者均适用9%的低税率，但在税法上是并列概念。购进香油不能开具农产品收购发票，也不能适用农产品的核定扣除或加计扣除政策，必须取得增值税专用发票等常规扣税凭证凭票抵扣。",
        "典型案例": "某企业向农户购进香油，自行开具农产品收购发票并以此计算抵扣进项税额，此操作不合规，面临税务稽查风险。"
    }},
    {{
        "触发条件": "当用户提问中涉及到羊粪等动物排泄物是否属于农产品及如何抵扣进项时", 
        "操作指引": "羊粪在税法上属于农业废弃物，不属于初级农产品。因此购进羊粪不能适用农产品的特定抵扣规则（如自行开具农产品收购发票抵扣）。此外，若购进羊粪是用于生产免征增值税的有机肥，根据规定，用于免税项目的进项税额一律不得抵扣。",
        "典型案例": "化肥厂直接向养殖户收购羊粪用于生产有机肥，企业开具了农产品收购发票并抵扣了进项税额。由于羊粪不是农产品，且用于免税项目，该笔进项税额必须做转出处理。"
    }}
]"""
